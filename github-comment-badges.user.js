...
;(async function() {
  ...
  async function processDetailsPage() {
    console.log('processDetailsPage')
    const repoData = await getRepoData()
    const pullRequestId = /pull\/([0-9]+)/.exec(window.location.pathname)[1]
    const messageCount = parseInt(
      document.querySelector('#conversation_tab_counter').innerText,
      10,
    )
    ...
    if (Number.isInteger(messageCount)) {
      const previousMessageCount = repoData[pullRequestId]
      if (messageCount !== previousMessageCount) {
        setRepoData({ ...repoData, [pullRequestId]: messageCount })
      }
    }
  }
  ...
  function isListPage() {
    const { section } = getInfoFromUrl()
    return section === 'pulls' || section === 'issues'
  }
  ...
  function getInfoFromUrl() {
    const [repoOwner, repo, section] = window.location.pathname
      .substr(1)
      .split('/')
    return {
      repoOwner,
